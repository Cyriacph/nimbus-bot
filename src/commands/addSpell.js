...
const SPELLBOOK_PATH = path.join(
  __dirname,
  "../../rule_documents/text_documents/spellbook.json",
);
...
module.exports = {
  name: "addSpell",
  description:
    "Add a spell to a character sheet by name, copying the spell description from the spellbook.",
  async execute(message, args) {
    ...
    if (!foundSpell) {
      return message.reply(
        `Spell "${args.slice(1).join(" ")}" not found in the spellbook.`,
      );
    }
    ...
    try {
      ...
      return message.reply(
        `Could not load character sheet for '${characterName}'.`,
      );
    }
    ...
    if (
      sheet.spells.some(
        (s) => s.name && s.name.toLowerCase() === foundSpell.name.toLowerCase(),
      )
    ) {
      return message.reply(
        `'${characterName}' already knows the spell '${foundSpell.name}'.`,
      );
    }
    ...
    return message.reply(
      `Added spell '${foundSpell.name}' to '${characterName}'.`,
    );
  },
};
