...
function handleConditionCommand(message, args) {
  ...
  if (!CONDITIONS_DB.hasOwnProperty(rawConditionName)) {
    ...
    message.reply(
      `Invalid condition '${rawConditionName}'. Valid conditions are: ${validConditions}`,
    );
    return;
  }
  ...
  try {
    ...
    message.reply(
      `Condition '${rawConditionName}' has been ${actionTaken} ${characterName}.\n**Description:** ${description}`,
    );
  } catch (err) {
