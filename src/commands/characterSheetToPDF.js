...
const CHARACTER_SHEET_DIR = path.join(
  __dirname,
  "..",
  "..",
  "character_sheets",
...
function characterSheetToPDF(characterName, callback) {
  ...
  function getSpells(data) {
    if (
      ...
      Array.isArray(data.character_sheet.spells) &&
      data.character_sheet.spells.length > 0
    ) {
  ...
  doc
    .fontSize(18)
    .text(
      `${SECTION_HEADERS.CHARACTER_SHEET}: ${sheet.basic_details.character_name}`,
      { underline: true },
    );
  doc.moveDown();
  doc.fontSize(12);
  ...
  doc.moveDown();
  doc.text(
    `${SECTION_HEADERS.HP}: ${sheet.hit_points.current}/${sheet.hit_points.max} (Temp: ${sheet.hit_points.temp_hp})`,
  );
  doc.text(
    `${SECTION_HEADERS.WOUNDS}: ${sheet.wounds.current}/${sheet.wounds.max}`,
  );
  ...
  doc.text(
    `${SECTION_HEADERS.INVENTORY_SLOTS}: ${sheet.inventory_slots.used}/${sheet.inventory_slots.total}`,
  );
  doc.moveDown();
  ...
  Object.entries(sheet.stats).forEach(([stat, val]) => {
    doc.text(
      `  ${stat.charAt(0).toUpperCase() + stat.slice(1)}: ${val.value} (Save: ${val.save})`,
    );
  });
  doc.moveDown();
  ...
  Object.entries(sheet.skills).forEach(([skill, skillObj]) => {
    ...
    doc.text(
      `  ${skill.charAt(0).toUpperCase() + skill.slice(1)}: ${skillValue}`,
    );
  });
  doc.moveDown();
  ...
  (sheet.equipment || []).forEach((item) => {
    doc.text(
      `  ${item.name} (${item.type}) - ${item.properties} ${item.damage ? "Damage: " + item.damage : ""}`,
    );
  });
  doc.moveDown();
  ...
  // Languages
  doc.text(
    `${SECTION_HEADERS.LANGUAGES} ${(sheet.languages || []).join(", ")}`,
  );
  doc.moveDown();
