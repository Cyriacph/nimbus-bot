  ...
  execute: async (message, args) => {
    if (!args.length) {
      return message.reply(
        "Please provide the name of the spell to search for.",
      );
    }
    ...
    if (!foundSpell) {
      return message.reply(
        `Spell "${args.join(" ")}" not found in the spellbook.`,
      );
    }
    ...
    if (foundSpell.high_levels)
      response += `*High Levels:* ${foundSpell.high_levels}\n`;
