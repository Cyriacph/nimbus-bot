...
function roll(notation) {
  ...
  if (numDice < MIN_DICE || numDice > MAX_DICE) {
    throw new DiceRangeError(
      ERROR_MESSAGES.DICE_RANGE.replace("{min}", MIN_DICE).replace(
        "{max}",
        MAX_DICE,
      ),
    );
  ...
  if (!isValidDie(numFaces)) {
    throw new InvalidDieError(
      ERROR_MESSAGES.UNSUPPORTED_DIE.replace("{faces}", numFaces),
    );
  }
